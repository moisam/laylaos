...
#
# Script to download and build automake
#
...
check_existing ${DOWNLOAD_NAME} ${CROSSCOMPILE_SYSROOT_PATH}/usr/bin/automake
...
# patch these suckers
sed -i '1 s/^.*$/#! \/usr\/bin\/perl/' ${CROSSCOMPILE_SYSROOT_PATH}/usr/bin/aclocal-1.17
sed -i '1 s/^.*$/#! \/usr\/bin\/perl/' ${CROSSCOMPILE_SYSROOT_PATH}/usr/bin/aclocal
sed -i '1 s/^.*$/#! \/usr\/bin\/perl/' ${CROSSCOMPILE_SYSROOT_PATH}/usr/bin/automake-1.17
