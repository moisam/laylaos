...
#
# Script to download and build groff
#

DOWNLOAD_NAME="groff"
DOWNLOAD_VERSION="1.23.0"
DOWNLOAD_URL="https://ftp.gnu.org/gnu/groff/"
DOWNLOAD_PREFIX="groff-"
...
check_existing ${DOWNLOAD_NAME} ${CROSSCOMPILE_SYSROOT_PATH}/usr/bin/groff
...
CFLAGS="$CFLAGS -mstackrealign" CXXFLAGS="$CXXFLAGS -mstackrealign -fPIC -DPIC" \
    ../configure \
    --host=${BUILD_TARGET} --prefix=/usr \
    || exit_failure "$0: failed to configure ${DOWNLOAD_NAME}"

make TROFFBIN=troff GROFFBIN=groff GROFF_BIN_PATH= || exit_failure "$0: failed to build ${DOWNLOAD_NAME}"
