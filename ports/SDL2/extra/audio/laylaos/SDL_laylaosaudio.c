...
/*
 * Driver for LaylaOS audio(4).
 * mohammed_isam1984@yahoo.com.
 *
 * Based on the driver for native OpenBSD/NetBSD audio(4).
 * vedge@vedge.com.ar.
 */
...
static void
LAYLAOSAUDIO_DetectDevices(void)
{
...
LAYLAOSAUDIO_Status(_THIS)
{
    ...
    //info.mode = (this->iscapture ? AUMODE_RECORD : AUMODE_PLAY);
...
static int
LAYLAOSAUDIO_OpenDevice(_THIS, const char *devname)
{
    ...
    for(format = SDL_FirstAudioFormat(this->spec.format); format; format = SDL_NextAudioFormat())
    {
        switch(format)
        {
        case AUDIO_U8:
...
static void
LAYLAOSAUDIO_CloseDevice(_THIS)
{
    if(this->hidden->audio_fd >= 0)
    {
...
static int
LAYLAOSAUDIO_CaptureFromDevice(_THIS, void *_buffer, int buflen)
{
...
static void
LAYLAOSAUDIO_FlushCapture(_THIS)
{
    audio_info_t info;
    size_t remain;
    Uint8 buf[512];
...
static SDL_bool
LAYLAOSAUDIO_Init(SDL_AudioDriverImpl * impl)
{
    ...
    impl->FlushCapture = LAYLAOSAUDIO_FlushCapture;
...
AudioBootStrap LAYLAOSAUDIO_bootstrap = {
    "laylaos", "LaylaOS audio", LAYLAOSAUDIO_Init, 0
};

#endif /* SDL_AUDIO_DRIVER_LAYLAOS */
